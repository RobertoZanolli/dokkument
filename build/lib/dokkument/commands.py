...
class Command(ABC):
    ...
    def __init__(self, link_manager: LinkManager, browser_opener: BrowserOpener, cli_display: CLIDisplay):
...
class OpenAllLinksCommand(Command):
    ...
    def execute(self) -> bool:
        ...
        self.cli_display.print_success_message(f"Aperti {success_count} link su {len(entries)}")
...
class ReloadCommand(Command):
    ...
    def execute(self) -> bool:
        ...
        try:
            ...
            self.cli_display.print_scan_results(total_links, len(self.link_manager.get_entries_by_file()))
...
class StatisticsCommand(Command):
    ...
    def execute(self) -> bool:
        ...
        if self.config.get('advanced.debug_mode', False):
            ...
            print("\n" + self.cli_display.colorize("= Dettagli Debug:", 'info'))
            for file_path, entries in entries_by_file.items():
                print(f"  =Ä {file_path}: {len(entries)} link")
    ...
    def execute(self, action: str = 'show') -> bool:
        if action == 'show':
            ...
            if self.config.export_config_template(template_path):
                self.cli_display.print_success_message(f"Template esportato in: {template_path}")
            else:
                self.cli_display.print_error_message("Errore nell'esportazione del template")
        elif action == 'validate':
            errors = self.config.validate_config()
            if errors:
                self.cli_display.print_error_message("Errori di configurazione trovati:")
                for error in errors:
                    print(f"  " {error}")
            else:
...
class ValidateLinksCommand(Command):
    ...
    def execute(self) -> bool:
        ...
        if invalid_links:
            self.cli_display.print_warning_message(f"Trovati {len(invalid_links)} link non validi:")
            for entry, error in invalid_links:
                print(f"  L {entry.description}: {error}")
                print(f"     =Ä File: {entry.file_path}")
...
class ExportCommand(Command):
    ...
    def execute(self, format_type: str = 'text', output_file: Optional[str] = None) -> bool:
        try:
            ...
            if output_file:
                ...
                with open(output_path, 'w', encoding='utf-8') as f:
                    f.write(content)
                self.cli_display.print_success_message(f"Link esportati in: {output_path}")
            else:
                print(content)
...
class SearchCommand(Command):
    ...
    def execute(self, search_term: str) -> bool:
        ...
        if not matching_entries:
            self.cli_display.print_warning_message(f"Nessun link trovato per: '{search_term}'")
        else:
    ...
    def execute_command(self, command_name: str, *args, **kwargs) -> bool:
        ...
        if command:
            try:
                ...
                self.cli_display.print_error_message(f"Errore nell'esecuzione del comando: {e}")
                return True
        else:
            self.cli_display.print_error_message(f"Comando non riconosciuto: {command_name}")
            return True
    ...
    def parse_and_execute_user_input(self, user_input: str, total_entries: int) -> bool:
        ...
        if user_input.isdigit():
            ...
            if 1 <= index <= total_entries:
                return self.execute_command('open_link', index)
            else:
                self.cli_display.print_error_message(f"Numero non valido. Inserisci un numero tra 1 e {total_entries}")
                return True
        ...
        self.cli_display.print_error_message(f"Comando non riconosciuto: '{user_input}'")
