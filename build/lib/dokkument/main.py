...
class DokkumentApp:
    ...
    def run_interactive_mode(self, scan_path: Path = None):
        """
        Esegue l'applicazione in modalità interattiva
    ...
    def run_list_mode(self, scan_path: Path = None, format_type: str = 'text'):
        """
        Esegue l'applicazione in modalità lista (non interattiva)
    ...
    def run_open_mode(self, scan_path: Path = None, link_indices: list = None, open_all: bool = False):
        """
        Esegue l'applicazione in modalità apertura diretta
...
Esempi di uso:
  ...
  dokkument --config show          # Mostra configurazione
    ...
    # Modalità operative (mutuamente esclusive)
    mode_group = parser.add_mutually_exclusive_group()
    
    mode_group.add_argument(
        '--list', '-l',
        action='store_true',
        help='Mostra solo la lista dei link senza modalità interattiva'
    )
    
    mode_group.add_argument(
        '--open-all', '-a',
        action='store_true',
        help='Apre tutti i link trovati e esce'
    )
    ...
    # Opzioni per la modalità lista
    parser.add_argument(
        '--format', '-f',
        choices=['text', 'json', 'markdown', 'html'],
        default='text',
        help='Formato di output per la modalità lista (default: text)'
    )
    ...
    parser.add_argument(
        '--no-color',
        action='store_true',
        help='Disabilita i colori nell\'output'
    )
    
    parser.add_argument(
        '--debug',
        action='store_true',
        help='Abilita la modalità debug con informazioni aggiuntive'
    )
    
    parser.add_argument(
        '--version', '-v',
        action='version',
        version='dokkument 1.0.0'
    )
    ...
    parser.add_argument(
        '--validate',
        action='store_true',
        help='Valida tutti i link trovati'
    )
