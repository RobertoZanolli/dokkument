...
class DokkEntry:
    ...
    def _validate(self):
        ...
        if not self.url.startswith(('http://', 'https://')):
            raise ParseError(f"URL non valido '{self.url}' per '{self.description}' in {self.file_path}")
        ...
        for line in content.splitlines():
            ...
            if match:
                ...
                try:
                    ...
                    raise ParseError(f"Errore alla riga {line_number} di {file_path}: {e}")
            else:
                raise ParseError(f"Formato non valido alla riga {line_number} di {file_path}: {line}")
...
class DokkFileScanner:
    ...
    def scan_directory(self, root_path: Path, recursive: bool = True) -> Dict[Path, List[DokkEntry]]:
        """
        ...
        if not root_path.is_dir():
            raise NotADirectoryError(f"Il path specificato non è una directory: {root_path}")
    ...
    def get_all_entries(self, root_path: Path, recursive: bool = True) -> List[DokkEntry]:
        """
